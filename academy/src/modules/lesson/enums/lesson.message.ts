export enum LessonMessages {
  CREATE_SUCCESS = 'Lesson created successfully',
  CREATE_FAILURE = 'Failed to create lesson. Try again later',

  UPDATE_SUCCESS = 'Lesson updated successfully',
  UPDATE_FAILURE = 'Failed to update lesson. Try again',

  GET_SUCCESS = 'Lesson retrieved successfully',
  GET_FAILURE = 'Failed to retrieve lesson',

  GET_ALL_SUCCESS = 'Lessons retrieved successfully',
  GET_ALL_FAILURE = 'Failed to retrieve lessons',

  REMOVE_SUCCESS = 'Lesson removed successfully',
  REMOVE_FAILURE = 'Failed to remove lesson',

  NOT_FOUND = 'Lesson not found',
}
