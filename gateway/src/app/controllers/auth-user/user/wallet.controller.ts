...
export class WalletController {
  ...
  constructor(
    @Inject(Services.AUTH_USER) private readonly userServiceClient: ClientProxy,
  ...
  async pay(@Body() paymentDto: PaymentDto, @GetUser() user: User) {
    try {
      ...
      await checkConnection(Services.AUTH_USER, this.userServiceClient);
      ...
      handleError(error, 'Failed to pay', Services.AUTH_USER);
    }
  }
  ...
  async verify(@Query('Authority') authority: string, @Query('Status') status: string, @GetUser() user: User) {
    try {
      ...
      handleError(error, 'Failed to verify pay', Services.AUTH_USER);
    }
  }
  ...
  async manualCreditWallet(@Param('walletId', ParseIntPipe) walletId: number, @Body() manualCreditDto: ManualCreditDto, @GetUser() user: User) {
    try {
      ...
      handleError(error, 'Failed to manual credit wallet', Services.AUTH_USER);
    }
  }
  ...
  async findAll(@Query() walletFilters: QueryWalletsDto) {
    try {
      ...
      handleError(error, 'Failed to get wallets', Services.AUTH_USER);
    }
  }
  ...
  async findAllDeductions(@Query() deductionFilers: QueryWalletDeductionsDto) {
    try {
      ...
      handleError(error, 'Failed to get all deductions', Services.AUTH_USER);
    }
  }
  ...
  async findAllManualCredits(@Query() manualCreditFilters: QueryManualCreditsDto) {
    try {
      ...
      handleError(error, 'Failed to get all manual credits', Services.AUTH_USER);
    }
  }
  ...
  async getMyWallet(@GetUser() user: User) {
    try {
      ...
      handleError(error, 'Failed to get my wallet', Services.AUTH_USER);
    }
  }
  ...
  async findOne(@Param('id', ParseIntPipe) id: number) {
    try {
      ...
      handleError(error, 'Failed to get wallet', Services.AUTH_USER);
    }
  }
  ...
  async block(@Param('id', ParseIntPipe) id: number) {
    try {
      ...
      handleError(error, 'Failed to block wallet', Services.AUTH_USER);
    }
  }
  ...
  async unblock(@Param('id', ParseIntPipe) id: number) {
    try {
      ...
      handleError(error, 'Failed to unblock wallet', Services.AUTH_USER);
    }
  }
}
