...
export class AuthService {
  ...
  async signin(signinDto: ISignin): Promise<ServiceResponse> {
    try {
      const result = await this.userService.findByIdentifier(signinDto);
  ...
  private async ensureUserDoesNotExist(mobile: string, username: string): Promise<void | never> {
    ...
    const userRes = await this.userService.findByArgs(userArgs);
  ...
  private async updateUser(userId: number, data: Omit<IUpdateUser, 'userId'>): Promise<ServiceResponse> {
